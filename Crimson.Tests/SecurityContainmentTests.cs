...
public sealed class SecurityContainmentTests
{
    ...
    [Fact]
    public void StorageKeyCodec_ProducesDistinctPathSafeKeysForAdversarialIdentifiers()
    {
        var values = new[]
        {
            "../escape",
        ...
        var keys = values.Select(StorageKeyCodec.Encode).ToArray();

        Assert.Equal(values.Length, keys.Distinct(StringComparer.Ordinal).Count());
        Assert.All(keys, key =>
        {
