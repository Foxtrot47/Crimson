...
public sealed class ParserAdversaryTests
{
    ...
    [Fact]
    public void ManifestRead_RejectsTrailingDataAndUnsupportedVersion()
    {
        var valid = BuildManifest(Encoding.UTF8.GetBytes("bounded manifest"));
        var withTrailingData = valid.Concat(new byte[] { 0xFF }).ToArray();
        var unsupportedVersion = valid.ToArray();
        BitConverter.GetBytes(22).CopyTo(unsupportedVersion, 37);

        Assert.Throws<InvalidDataException>(() => Manifest.Read(withTrailingData));
        Assert.Throws<InvalidDataException>(() => Manifest.Read(unsupportedVersion));
    }
    ...
    [Fact]
    public void SectionParsers_RejectNegativeCountsAndPathCollisions()
    {
        ...
        Assert.Throws<InvalidDataException>(() => FileManifestList.Read(BuildDuplicatePathSection()));
    }
