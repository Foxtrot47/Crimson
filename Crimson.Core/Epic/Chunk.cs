...
public sealed class Chunk
{
    ...
    public byte[] Data
    {
        get
        {
            ...
            if (_data.Length != UncompressedSize)
                throw new InvalidDataException("Chunk uncompressed size does not match its payload.");
            if (RollingHash.ComputeHash(_data) != Hash)
                throw new InvalidDataException("Chunk rolling hash does not match its payload.");
            if (HeaderVersion >= 2 &&
                !CryptographicOperations.FixedTimeEquals(SHA1.HashData(_data), ShaHash))
                throw new InvalidDataException("Chunk SHA-1 does not match its payload.");
