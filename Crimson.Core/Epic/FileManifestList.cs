...
public sealed class FileManifestList
{
    ...
    internal static FileManifestList Read(EpicBinaryReader reader)
    {
        ...
        foreach (var file in list.Elements)
        {
            ...
            if ((file.Flags & ~0x7) != 0)
                throw new InvalidDataException("File manifest flags are unsupported.");
        }
        ...
        foreach (var file in list.Elements)
        {
            ...
            for (var index = 0; index < tagCount; index++)
                file.InstallTags.Add(reader.ReadUtf8String());
        }
        ...
        if (list.Version >= 1)
        {
            ...
            foreach (var file in list.Elements)
                file.MimeType = reader.ReadUtf8String();
        }
        ...
        reader.EndSection(end, "File manifest list");
        return list;
    }
}
